...
//        URI url = "http://localhost:8080/medicos/"+medico.getId()
    ...
    @GetMapping
    public ResponseEntity<Page<DatosListadoMedico>> listarMedicosActivos(@PageableDefault(size = 2) Pageable paginacion) {
        Short activo = 1; // valor que representa "activo"
        return ResponseEntity.ok(medicoRepository.findByActivo(activo, paginacion).map(DatosListadoMedico::new));
    }
